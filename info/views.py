...
def contact_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                'تم إرسال رسالتك بنجاح، وسيرد عليك الطبيب قريبًا.'
            )
...
def contact_history_view(request):
    """عرض سجل الرسائل المرسلة من المستخدم مع الردود إن وجدت."""
    msgs = ContactMessage.objects.filter(user=request.user) \
                                .order_by('-created_at')
    return render(
        request,
        'info/contact_history.html',
        {'messages': msgs}
    )
...
def admin_dashboard(request):
    ...
    # آخر 6 أشهر بدءًا من أول يوم للشهر
    now = timezone.now()
    ...
    for _ in range(6):
        ...
        rec = next(
            (item for item in stats
             if item['month'].year == month_cursor.year
             and item['month'].month == month_cursor.month),
            None
        )
...
def admin_message_detail(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # محاولة إرسال بريد الإخطار للمستخدم
            try:
                send_mail(
            ...
            messages.success(
                request,
                'تم حفظ الرد وإخطار المستخدم عبر البريد.'
            )
    ...
    return render(
        request,
        'info/admin/message_detail.html',
        {'message_obj': msg, 'form': form}
    )
...
def resource_confirm_delete_view(request, pk):
    ...
    return render(
        request,
        'info/resource_confirm_delete.html',
        {'resource': resource}
    )
